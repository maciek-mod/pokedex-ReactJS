...
const DetailsPokemon = (props) => {
    return (<div className="details_pokemon">
        ...
        <div className="sprites">
            ...
            {
                props.sprites.back_shiny && <button onClick={props.toggleClass.bind(this, "toggleSprite")}>{
                            props.toggleClassNew.toggleSprite
                                ? "Hide shiny form"
                                : "Show shiny form"
                        }</button>
            }
            ...
            <div className="type">
                {
                    props.types.map((item, key) => {
                        return (<Link key={key} to={"/type" + item.type.url.match(/\/\d+/)}>
                            <p className={item.type.name}>{props.capitalizeFirstLetter(item.type.name)}</p>
                        </Link>);
                    })
                }
            </div>
        ...
        <div className="details_pokemon_data">
            ...
            <div className="item">
                <h2>{props.numberWithCommas(props.weight)}
                    kg</h2>
                <p>Weight</p>
            </div>
        </div>
        ...
        <div className="section_button">
            <div className="section_button_buttons">
                <h2 className={(
                        props.toggleClassNew.toggleAbilities
                        ? "active"
                        : "")} onClick={props.toggleClass.bind(this, "toggleAbilities")}>Abilities</h2>
                <h2 className={(
                        props.toggleClassNew.toggleStats
                        ? "active"
                        : "")} onClick={props.toggleClass.bind(this, "toggleStats")}>Stats</h2>
                <h2 className={(
                        props.toggleClassNew.toggleMoves
                        ? "active"
                        : "")} onClick={props.toggleClass.bind(this, "toggleMoves")}>Moves</h2>
            </div>
                    ...
                    : "")}>
                {
                    props.stats.map((item, key) => {
                        return (<p key={key}>
                            ...
                            <br/> {props.capitalizeFirstLetter(item.stat.name)}
                        </p>);
                    })
                }
            </div>
            <div className={"item moves" + " " + (
                    props.toggleClassNew.toggleMoves
                    ? "show"
                    : "")}>
                {
...
DetailsPokemon.propTypes = {
    ...
    toggleClassNew: PropTypes.object.isRequired,
